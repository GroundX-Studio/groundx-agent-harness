...
The default wrapper templates are generated by `compile_workflow.py`. For a
pilot that needs domain-specific wrapper wording, put that wording behind a
small manager based on `templates/prompt_manager.py`; the harness compiler does
not dynamically load wrapper modules.
